...
- **Camera-to-room mapping is discovery-driven** — No hard-coded camera list. Cameras are found via `camera.*` entities in HA registry, room resolved via device→area chain. Manual overrides via `presence.camera_rooms` config key. Mapping merges on re-discovery (never resets).
