...
| `presence` | `aria/modules/presence.py` | Subscribes to Frigate MQTT (person/face detection) and HA WebSocket (motion sensors, lights, dimmers, media players, device trackers, door sensors). Seeds room signals on startup from all presence-relevant domains (light.*, binary_sensor.*, media_player.*, event.*) to fix cold-start. Uses local entity→room cache (O(1) dict lookup) + hub cache fallback to avoid slow subscriber warnings. Feeds signals into BayesianOccupancy for per-room presence estimation. Media player states (playing/off) contribute to room probability signals. |
